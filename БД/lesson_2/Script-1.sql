...
-- комментарии под различными медиа
...
-- имеется ввиду примитивная стена, как раньше была, например, вк, а не современный микроблог
-- пока логика такая, что пользователь может прикрепить медиа или нет, но какой-то текст написать обязан
DROP TABLE IF EXISTS `wall`;
CREATE TABLE `wall` (
	id SERIAL PRIMARY KEY,
	wall_user_id BIGINT UNSIGNED NOT NULL,
...
-- пока имеется ввиду не музыка у пользователя на странице, а общий раздел соц.сети с музыкой
-- не уверен на счет большого числа индексов, но по логике поиск может осущесвляться как по 
-- группе, альбому и названию, так и индексы нужно повесить на внешние ключи медиа и юзер
